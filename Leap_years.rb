 ...
 year, and then puts all of the leap years between them (and including them,
  ...
  1984 and 2004). However, years divisible by 100 are not leap years (such as
   1800 and 1900) unless they are divisible by 400 (like 1600 and 2000, which
    were in fact leap years). (Yes, it's all pretty confusing, but not as
     confusing as having July in the middle of the winter, which is what would
      eventually happen.)
=end

puts "What year will it start?"
start_year = gets.chomp.to_i
puts "What year will it stop?"
end_year = gets.chomp.to_i

for leap_year in (start_year..end_year)
    if leap_year % 4 == 0
        puts leap_year
        else
        puts ""
